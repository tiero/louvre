...
export class TradeHandler {
	...
	async marketPrice(call: any, callback: any) {
		...
		if (!call.request) return callback({
			code: status.INVALID_ARGUMENT,
			message: 'Malformed request',
		});

	
		if (!call.request.hasMarket()) return callback({
			code: status.INVALID_ARGUMENT,
			message: 'Missing market',
		});
		const market = call.request.getMarket()!;
		...
		try {
			const { Price, Balance, Fee, Amount, Asset } = await this.tradeService.getMarketPrice(
				{
					BaseAsset: market.getBaseAsset(),
					QuoteAsset: market.getQuoteAsset(),
				},
		...
		try {
			const result = await this.tradeService.proposeTrade(
				...
				},
				{
					...
					InputBlindingKeyByScript: blindKeysMap(swapRequest.getInputBlindingKeyMap()),
					OutputBlindingKeyByScript: blindKeysMap(swapRequest.getOutputBlindingKeyMap()),
				}
			);
			...
			Object.entries(result.acceptTx!.InputBlindingKeyByScript).forEach(([key, value]) => {
				swapAccept.getInputBlindingKeyMap().set(key, Uint8Array.from(value));
			});


			// set the output blinding keys
			Object.entries(result.acceptTx!.OutputBlindingKeyByScript).forEach(([key, value]) => {
	...
	async completeTrade(
		call: ServerUnaryCall<tradeMessages.CompleteTradeRequest, tradeMessages.CompleteTradeReply>,
		...
		if (!call.request.hasSwapComplete()) return callback({
			code: status.INVALID_ARGUMENT,
		...
		try {
			const result = await this.tradeService.completeTrade(swapComplete.getTransaction());
